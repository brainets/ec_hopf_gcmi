...
% by adjusting the Feature-specific Information Transfer (FIT) to the model
...
function eff_hopf_gcmi_FIT(simulation, reaction, condition)

rng shuffle; % necessary to generate random numbers in the cluster

addpath('../data');

% empirical data calculated with FIT
load('fit_difficulty_bis.mat');
signal_fit=fit_matrix;
fitm=squeeze(nanmean(fit_matrix(:,:,:),3));
fitm(isnan(fitm))=0;
...
save (sprintf('results_eff_hopf_GC_FIT_%s_%s_%s.mat', num2str(condition),num2str(reaction),num2str(simulation)),'fcsimuli','FCemp', ...
    'corr2FCSC','corr2FCemp_sim','fittFC','iEffectiveConnectivity','Cnew','GCsim2');
end